...
inline void delay_nsec(uint32_t n)
{
    n *= 0x2aaa;                // divide by 6.
    n >>= 16;
    while (--n) {
        __asm volatile("nop");
    }
...
#define LCD_DATA_PINS (GPIO15 | GPIO14 | GPIO13 | GPIO12 |      \
...
class ILI9341_t3 // : public Print
{
 ...
 protected:
...
#if LCD_DATA_PINS == 0xFF00
            *(volatile uint8_t *)((uintptr_t)&GPIO_ODR(LCD_DATA_PORT) + 1) = c;
#else
            #error "data pins should be high byte."
#endif
            // delay_nsec(30);
	...
	void writedata8_cont(uint8_t d) __attribute__((always_inline))
        {
             gpio_clear(LCD_WRX_PORT, LCD_WRX_PIN);
#if LCD_DATA_PINS == 0xFF00
            *(volatile uint8_t *)((uintptr_t)&GPIO_ODR(LCD_DATA_PORT) + 1) = d;
#else
            #error "data pins should be high byte."
#endif
            // delay_nsec(30);
	...
	void writedata16_cont(uint16_t d) __attribute__((always_inline))
        {
            writedata8_cont(d >> 8); // MSByte first?
            writedata8_cont(d & 0xFF);
	}
...
private:
        ILI9341_t3(const ILI9341_t3&);
        ILI9341_t3& operator =(const ILI9341_t3&);
};
...
public:
	Adafruit_GFX_Button(void) : _gfx(NULL), _x(0), _y(0), _w(0), _h(0), _textsize(0),_outlinecolor(0), _fillcolor(0), _textcolor(0), currstate(false), laststate(false) {}
